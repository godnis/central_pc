    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                @can('editar')
                    <a href="{{ route('componentes.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium shadow-sm hover:bg-brand-700">
                        {{ __('Novo componente') }}
                    </a>
                @endcan
            </div>
                        ...
                        @forelse ($componentes as $componente)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                    @can('editar')
                                        <a href="{{ route('componentes.edit', $componente) }}" class="text-brand-600 hover:underline">{{ __('Editar') }}</a>
                                    @endcan
                                    @can('excluir')
