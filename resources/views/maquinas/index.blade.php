        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @endcan
                    @can('editar')
                        <a href="{{ route('maquinas.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-medium shadow-sm hover:bg-brand-700">
                            {{ __('Nova máquina') }}
                        </a>
                    @endcan
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">{{ __('Placa-mãe') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($maquinas as $maquina)
                            ...
                            <tr>
                                <td class="px-6 py-4">
                                    <a href="{{ route('maquinas.show', $maquina) }}" class="text-brand-600 hover:underline">{{ $maquina->nome }}</a>
                                </td>
                                <td class="px-6 py-4 font-mono text-xs">{{ $maquina->patrimonio ?? '—' }}</td>
                                <td class="px-6 py-4">
                                    <span @class([
                                ...
                                <td class="px-6 py-4">
                                    @if ($placaMae)
                                        {{ $placaMae->nome }}
                                    @else
                                        <span class="whitespace-nowrap text-amber-600">{{ __('a definir') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                    @can('editar')
                                        <a href="{{ route('maquinas.edit', $maquina) }}" class="text-brand-600 hover:underline">{{ __('Editar') }}</a>
                                    @endcan
                                    @can('excluir')
